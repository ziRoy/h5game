...
var HeroController = (function () {
    ...
    var __egretProto__ = HeroController.prototype;
    /* 加英雄 或者更新英雄等级 */
    __egretProto__.createHero = function (heroId, level) {
    ...
    __egretProto__.findHero = function (heroId) {
        for (var i = 0; i < Game.role.herolist.length; i++) {
            if (Game.role.herolist[i].id == heroId)
                return Game.role.herolist[i];
        }
        return null;
    };
    /* 更换英雄 */
    __egretProto__.changeHero = function (preHeroId, curHeroId) {
    ...
    /*  升级英雄 */
    __egretProto__.upgradeHero = function (heroId) {
        var hero = this.findHero(heroId);
        if (!hero) {
